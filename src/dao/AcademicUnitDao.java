...
public class AcademicUnitDao {
    ...
    public AcademicUnit findByNames(String name) {
        try (Session session = HibernateUtility.getSession().openSession()) {
        	Query query = session.createQuery("select academic from AcademicUnit academic where academic.name = :name");
    ...
    public AcademicUnit findById(String id) {
        try (Session session = HibernateUtility.getSession().openSession()) {
        	Query query = session.createQuery("select academic from AcademicUnit academic where academic.code = :id");
    ...
    public List<AcademicUnit> findByUnitType(String type){
		...
		Query query = session.createQuery("select academic from AcademicUnit academic where academic.academicUnit = :type");
